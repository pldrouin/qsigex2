      SUBROUTINE LSQASM(USERFN,T,Y,DELTAY,N,NR,NRED,LIST,X0,CX,R,W,
     +DXPLUS,DXMINS,A,SCRAT,NSTEP)
      ...
      COMMON /DASV03/ X,DX
      PARAMETER(MAXSTP=100,EPS=1.D-2,EPSILN=1.D-4,TT=1.D-6,FACT=1.5D0)
...
C set XSMALL to x at minimum position
          ...
          DO 30 I=1,NSTEP
            ...
            CALL MTXCPV(X0,X,NR)
            X(IVAR)=XIVAR
            CALL LSQMAR(USERFN,T,Y,DELTAY,N,NR,NRED1,LIST,X,CX,R1,A,
     +      SCRAT,NSTEPL)
            IF(NSTEPL.LT.1) THEN
              NSTEP=-1
              GO TO 70
