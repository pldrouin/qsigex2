...
template <typename U> void QList<U>::Add(const QList<U>& newqlist, Int_t index)
{
...
template <typename U> void QList<U>::Add(const U& newelement,Int_t index)
{
...
template <typename U> void QList<U>::Add(const U* newelements, Int_t nelements, Int_t index)
{
...
template <typename U> void QList<U>::Set(const U* newelements, Int_t nelements)
{
...
template <typename U> Int_t QList<U>::Del(const QList<U>& delqlist,Int_t maxmatches)
{
...
template <typename U> Int_t QList<U>::Del(const U* delus, Int_t nelements, Int_t maxmatches)
{
...
template <typename U> QList<Int_t> QList<U>::Find(const QList<U>& qlist,Int_t maxmatches) const
{
...
template <typename U> QList<Int_t> QList<U>::Find(const U& u,Int_t maxmatches) const
{
  // This function browses the QList<U> U list in a similar manner to the Del member function with
  // the same arguments. However, instead of deleting list parts and returning the number of
  // deletions, it returns a QList<Int_t> containing the matching indexes.
...
template <typename U> QList<Int_t> QList<U>::Find(const U* us, Int_t nelements, Int_t maxmatches) const
{
  // This function browses the QList<U> U list in a similar manner to the Del member function with
  // the same arguments. However, instead of deleting list parts and returning the number of
  // deletions, it returns a QList<Int_t> containing the matching indexes.
...
template <typename U> U& QList<U>::operator[](Int_t index) const
{
...
template <typename U> const QList<U>& QList<U>::operator()(Int_t index1,Int_t index2,Int_t step) const
{
  //N.B. step does nothing at this time

  PRINTF10(this,"\tconst QList<U>& QList<",typeid(U).name(),">::operator()(Int_t index1<",index1,">,Int_t index2<",index2,">,Int_t step<",step,">)\n")
...
template <typename U> Int_t QList<U>::RedimList(Int_t newdim,Int_t index)
{
...
template <typename U> Int_t QList<U>::RedimList(Int_t newdim,Int_t index,const U& fillvalue)
{
  ...
  if(fNElements>nebefore){
    ...
    for(i=index;i<index+fNElements-nebefore;i++) fUArray[i]=fillvalue;
