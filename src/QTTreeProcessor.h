...
class QTTreeProcessor
{
  public:
    ...
    void DelHist(Int_t index=-1){index=HistIndexToIndex(index); fProcs->Del(index); fSelProcs->Del(index);}
    void DelHist(const char *histname);
    void DelProc(Int_t index=-1){index=ProcIndexToIndex(index); fProcs->Del(index); fSelProcs->Del(index); fNRegularProcs--;}
    void DelProc(const char *procname);

    void Exec();

    Int_t FindParamIndex(const char *paramname) const;
    Int_t FindHistIndex(const char *histname) const;
    Int_t FindProcIndex(const char *procname) const;

    Int_t GetNParams() const{return fParams->Count();}
    Int_t GetNHists() const{return fProcs->Count()-fNRegularProcs;}
    Int_t GetNProcs() const{return fNRegularProcs;}

    const char* GetParamName(Int_t i) const{return (*fParamsNames)[i];}

    QNamedProc& GetHist(Int_t index) const{index=HistIndexToIndex(index); return (*fProcs)[index];}
    QNamedProc& GetHist(const char *histname) const;
    QNamedProc& GetProc(Int_t index) const{index=ProcIndexToIndex(index); return (*fProcs)[index];}
  ...
  protected:
    ...
    Int_t ProcIndexToIndex(Int_t index, Bool_t creationmode=kFALSE) const;
    Int_t HistIndexToIndex(Int_t index, Bool_t creationmode=kFALSE) const;

  private:
    QList<QNamedProc> *fProcs;           //-> QNamedProc objects
    QList<Bool_t>     *fSelProcs;        //-> List of selector processes
    Int_t             fNRegularProcs;     //   Number of regular proecesses (processes that do not generate histograms)
