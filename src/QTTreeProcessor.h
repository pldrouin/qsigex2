...
class QTTreeProcessor
{
  public:
    ...
    void DelHist(Int_t index){fHists->Del(index);}
    void DelHist(const char* histname);
    void DelHistInput(Int_t histindex, Int_t index){(*fHInputsNames)[histindex].Del(index);}
    void DelHistInput(const char* histname, Int_t index);
  ...
  private:
    ...
    QList<TObject*>        *fHists;      //! QDisTH Objects
    QList<QList<QNamedVar<TString> > > *fHInputsNames; //-> Inputs for QDisTH Objects
