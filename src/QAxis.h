...
class QAxis: public TNamed
{
  public:
    ...
    template <typename U> void Set(const Int_t &nbins, const U* bins){Clear(); fNBins=nbins; fMin=bins[0]; fMax=bins[nbins]; fBWidth=(fMax-fMin)/fNBins; fBins=new Double_t[nbins+1]; for(Int_t i=0; i<=nbins; i++) fBins[i]=bins[i];}
    void Set(const Int_t &nbins, const Double_t* bins){Clear(); fNBins=nbins; fMin=bins[0]; fMax=bins[nbins]; fBWidth=(fMax-fMin)/fNBins; fBins=new Double_t[nbins+1]; memcpy(fBins,bins,(nbins+1)*sizeof(Double_t));}
