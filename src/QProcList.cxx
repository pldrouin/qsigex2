...
QProcList::~QProcList()
{
  ...
  pthread_mutex_destroy(&fTWCMutex);
  pthread_cond_destroy(&fTWCond);
  delete fQPL;
  fQPL=NULL;
}
...
void QProcList::Exec() const
{
  ...
  pthread_mutex_lock(&fTWCMutex);
  pthread_cond_broadcast(&fTWCond);
  pthread_mutex_unlock(&fTWCMutex);

  if(fMTRNotify) {
    pthread_mutex_lock(&fMTRNotify->RNCMutex);
    if(fMTRNotify->RCountDown) {
      //printf("Waiting for notification %p\n",fMTRNotify);
      pthread_cond_wait(&fMTRNotify->RNCond,&fMTRNotify->RNCMutex);
      //printf("Received notification %p\n",fMTRNotify);
    }
    pthread_mutex_unlock(&fMTRNotify->RNCMutex);
  }
}
...
void QProcList::InitThreads()
{
  ...
  if(fNRNotify) {
    ...
    for(i=fNRNotify-1; i>=0; --i) {
      ...
      pthread_cond_init(&fRNotify[i].RNCond,NULL);
    }
  }
...
void QProcList::TerminateThreads()
{
  ...
  if(fRNotify) {
    for(i=fNRNotify-1; i>=0; --i) {
      ...
      pthread_cond_destroy(&fRNotify[i].RNCond);
    }
    delete[] fRNotify;
    fRNotify=NULL;
    fNRNotify=0;
...
void* QPLThread(void *args){
  ...
  for(;;) {
    ...
    if(!plist.fFirstChain) {
      //printf("No chain for thread\n");
      pthread_mutex_unlock(&plist.fChMutex);

      for(;;) {
	pthread_mutex_lock(&plist.fTWCMutex);

	if(!plist.fStopThreads) {
	  //printf("Thread will wait\n");
	  pthread_cond_wait(&plist.fTWCond,&plist.fTWCMutex);

	} else {
	  //printf("Thread will stop\n");
	  pthread_mutex_unlock(&plist.fTWCMutex);
	  return NULL;
	}
	pthread_mutex_lock(&plist.fChMutex);
        pthread_mutex_unlock(&plist.fTWCMutex);

	if(plist.fFirstChain) break;
	pthread_mutex_unlock(&plist.fChMutex);
      }
    }
	...
	if(!chain->DChains[i]->NRDOChains) {
      ...
      pthread_mutex_lock(&plist.fTWCMutex);
      pthread_cond_signal(&plist.fTWCond);
      pthread_mutex_unlock(&plist.fTWCMutex);
    }
	...
	pthread_cond_signal(&chain->RNotify->RNCond);
      }
