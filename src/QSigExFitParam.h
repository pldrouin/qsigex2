...
{
  public:
    QSigExFitParam(const char* name): QNamedVar<Double_t>(name,0.), fStartVal(new QNamedVar<Double_t>("Start Value",0.)), fMinVal(new QNamedVar<Double_t>("Minimum Value",-0.5e7)), fMaxVal(new QNamedVar<Double_t>("Maximum Value",0.5e7)), fStepVal(new QNamedVar<Double_t>("Step Value",1.)), fFixed(new QNamedVar<Bool_t>("Is Fixed",kFALSE)), fPlusFitError(new QNamedVar<Double_t>("Plus Fit Error",-1.0)), fMinusFitError(new QNamedVar<Double_t>("Minus Fit Error","-1.0")), fFreeParamIndex(new QNamedVar<Int_t>("Free Parameter Index",-1)) {}
    QSigExFitParam(): QNamedVar<Double_t>(), fStartVal(new QNamedVar<Double_t>("Start Value",0.)), fMinVal(new QNamedVar<Double_t>("Minimum Value",-0.5e7)), fMaxVal(new QNamedVar<Double_t>("Maximum Value",0.5e7)), fStepVal(new QNamedVar<Double_t>("Step Value",1.)), fFixed(new QNamedVar<Bool_t>("Is Fixed",kFALSE)), fPlusFitError(new QNamedVar<Double_t>("Plus Fit Error",-1.0)), fMinusFitError(new QNamedVar<Double_t>("Minus Fit Error","-1.0")), fFreeParamIndex(new QNamedVar<Int_t>("Free Parameter Index",-1)) {}
    ...
    void Setup(const Double_t &startval, const Double_t& stepval, const Double_t& minval=-0.5e7, const Double_t &maxval=0.5e7, Bool_t fix=kFALSE){*fStartVal=startval; *fStepVal=stepval; *fMinVal=minval; *fMaxVal=maxval; *fFixed=fix; if((Double_t&)*fMinVal == (Double_t&)*fMaxVal) *fFixed=kTRUE;}
    void SetFix(Bool_t fix=kTRUE){*fFixed=fix;}
    void SetStartVal(const Double_t& startval){*fStartVal=startval;}
    void SetMinVal(const Double_t& minval=-0.5e7){*fMinVal=minval; if((Double_t&)*fMinVal == (Double_t&)*fMaxVal) *fFixed=kTRUE;}
    void SetMaxVal(const Double_t& maxval=0.5e7){*fMaxVal=maxval; if((Double_t&)*fMinVal == (Double_t&)*fMaxVal) *fFixed=kTRUE;}
    void SetStepVal(const Double_t& stepval){*fStepVal=stepval;}
    void SetRange(const Double_t& minval=-0.5e7, const Double_t& maxval=0.5e7){*fMinVal=minval; *fMaxVal=maxval; if((Double_t&)*fMinVal == (Double_t&)*fMaxVal) *fFixed=kTRUE;}
